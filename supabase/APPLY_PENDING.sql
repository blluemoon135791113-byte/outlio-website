-- ===========================================================================
-- OUTLIO — PENDING MIGRATIONS (0070)
-- Updated 2026-08-30
--
-- Platform build M1: the workspace tenancy backbone. Adds workspaces,
-- memberships, invitations and per-workspace feature flags.
--
-- ⚠️ THIS TOUCHES NO EXISTING TABLE. Lead Engine, billing, extraction and
-- Hubble rows are untouched and stay user-scoped. Every existing user simply
-- gains a personal workspace they own, so nothing changes for them on deploy.
--
-- ONE THING HERE IS NOT ADDITIVE, AND YOU SHOULD KNOW ABOUT IT:
-- `handle_new_user()` is REPLACED so a workspace and an owner membership are
-- created in the same transaction as the profile. That function runs on every
-- signup. It keeps its original profile insert verbatim and only appends to it,
-- but if you want to be careful, sign up a throwaway account immediately after
-- applying and confirm it lands in `public.workspaces`.
--
-- All statements are idempotent — safe to re-run:
--   * the enum is created inside a guarded DO block
--   * CREATE TABLE / CREATE INDEX IF NOT EXISTS
--   * CREATE OR REPLACE FUNCTION, DROP TRIGGER/POLICY IF EXISTS before create
--   * the backfill is guarded by NOT EXISTS, so a second run inserts nothing
--
-- Nothing here drops, renames, or rewrites an existing value.
--
-- AFTER APPLYING, regenerate types so the hand-written declarations in
-- lib/workspaces/db.ts can be deleted:
--   npm run db:types
--
-- Then follow the instructions at the top of lib/workspaces/db.ts: have
-- platformDb() return createAdminClient() unchanged, drop PlatformDatabase,
-- and run `npm run typecheck` to catch anything that drifted.
-- ===========================================================================
