/*
 * Add `lei_number` to the qualification field allow-list.
 *
 ...
 *
 * `lei_number` is a GLEIF registry identifier: an official, permanent business
 * attribute with nothing personal in it. Sourced free from api.gleif.org.
 *
...
alter table public.qualification_rules
  add constraint qualification_rules_field_check check (field in (
     ...
     *
     * ⚠️ NO FREE SOURCE ANSWERS THESE TODAY. With paid enrichment removed they
     * resolve to unknown at zero cost until a legitimate free provider exists.
     * They stay in the vocabulary because stored historical evidence remains
     * valid and narrowing this constraint would strand existing rules.
     */
