...
| A4 | ⚠️ **OVERSTATED — corrected 2026-09-13.** Membership is hard-`DELETE`d, so no inactive record survives for attribution. | `supabase/migrations/0070_workspaces.sql:72-82`; `lib/workspaces/actions.ts:326-332` | See the correction below. The interactive half of this entry was wrong, and the background half is now fixed. Remaining: the `status` column, which needs a migration. |
| A5 | ✅ **FIXED 2026-09-13.** Published flows froze the publisher's authority — `stampSendAuthority` / `stampBillingUser` baked `actorAuthorized` and `userId` into the immutable version. | `lib/flows/definition.ts:429,463` | A departed member's send permission and credit account survived in every published flow until someone re-published. Now re-checked live via `memberMayNow` against `flow_versions.created_by`. No migration was needed. |

### ⚠️ Correction to A4, recorded 2026-09-13

**Phase 0 claimed a removed member "keeps a live session" and retains access. That is wrong**, and it was wrong in the alarming direction — the same failure this repository's own guards keep being written to prevent.

`assertWorkspacePermission` (`lib/workspaces/context.ts:235-249`) calls `resolve()` **directly and uncached**, and `resolve()` re-reads `workspace_memberships` on every call. A removed member is refused `ERR_FORBIDDEN` on their very next request. **Interactive workspace access already stops immediately.**

The spec's prescribed remedy is also unsuitable here, for two independent reasons:

1. **The SDK cannot do it.** `GoTrueAdminApi.signOut(jwt, scope)` takes a **JWT**, not a user id — verified in the installed `@supabase/auth-js`. An admin removing somebody does not hold their JWT, and there is no "revoke by user id" on that surface.
2. **It would be wrong even if it could.** Two tenancy models coexist here. The Lead Engine is `user_id`-scoped and personally theirs. Being removed from one workspace must not sign somebody out of their own account; §3's "revoke sessions" assumes one workspace per user.

**What genuinely remained** was background work acting as a removed member — and that is A5, now fixed. The only outstanding piece is the `status` column so an inactive membership survives for attribution, which needs a migration and should be scoped with CRM-DN-02/03.
