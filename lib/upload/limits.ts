...
export type ResolvedUploadLimits = {
  ...
  /**
   * Leads billed per credit. `null` means a flat 1 credit per extraction.
   * Priced by `lib/limits/credits.ts`; charged by the database once the run is
   * parsed and the real lead count is known.
   */
...
export function resolveUploadLimits(
  ...
  return {
    ...
    leadsPerCredit: limits?.leads_per_credit ?? null,
  }
}
