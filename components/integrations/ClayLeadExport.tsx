...
function ExportButton({ count }: { count: number }) {
  ...
  return (
    ...
    >
      {pending ? 'Exporting to Clay…' : `Export ${count.toLocaleString()} selected to Clay`}
    </button>
  )
}

export function ClayLeadExport({
  selectedLeadIds,
  clayConnected,
  onSuccess,
}: {
  ...
  const [state, action] = useActionState(exportSelectedLeadsToClayAction, INITIAL)
